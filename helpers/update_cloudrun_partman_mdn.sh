...
REGION="us-central1"
SERVICE_NAME="rmin-cloudsql-runpartman-c"

if [[ "$PROJECT_ID" == "uspscio-it-idc-9060-00d" ]]; then
  CLOUDSQL_CONNECTION_NAME="${PROJECT_ID}:${REGION}:idc-dev-rmin-mail-delivery-notification-c"
  ...
  CLOUDSQL_CONNECTION_NAME="${PROJECT_ID}:${REGION}:idc-sit-rmin-mail-delivery-notification-c"
  ...
  CLOUDSQL_CONNECTION_NAME="${PROJECT_ID}:${REGION}:idc-cat-rmin-mail-delivery-notification-c"
  
elif [[ "$PROJECT_ID" == "uspscio-it-idc-9060-01c" ]]; then
  CLOUDSQL_CONNECTION_NAME="${PROJECT_ID}:${REGION}:idc-cat-rmin-mail-delivery-notification-c"
  
elif [[ "$PROJECT_ID" == "uspscio-it-idc-9060-00p" ]]; then
  CLOUDSQL_CONNECTION_NAME="${PROJECT_ID}:${REGION}:idc-prod-rmin-mail-delivery-notification-c"
